...
import torch
import numpy as np
# sys.path.append("/home/work_nfs7/xcli/work/percepnet/c_spec/make")
...
def rnnoise_in(inputs, labels, nb_features=64, nb_bands=50, 
    ...
    inputs_real = np.real(inputs_spec) * 136
    inputs_imag = np.imag(inputs_spec) * 136
    ...
    labels_real = np.real(labels_spec) * 136
    labels_imag = np.imag(labels_spec) * 136
