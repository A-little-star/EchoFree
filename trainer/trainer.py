...
from pyrnnoise.rnnoise_module import MelFilterBank, vorbis_window, RnnoiseModule
...
class Trainer(object):
    ...
    def __init__(self,
        ...
        # assert not os.path.exists(self.checkpoint)
    ...
    def save_checkpoint_step(self, epoch, step):
        cpt = {
            "epoch": epoch,
            "model_state_dict": self.nnet.state_dict(),
            "optim_state_dict": self.optimizer.state_dict()
    ...
    def train(self, data_loader, epoch, max_epoch, lr):
        ...
        for egs in data_loader:
            ...
            # outputs = self.nnet(egs["mix"].clone(), egs["laec_echo"].clone())
            outputs = self.nnet(egs["mix"].clone(), egs["laec_echo"].clone())
            ...
            echo_ref_sig = egs["echo_ref_sig"]
            label = egs["label"]
            X = th.stft(label, n_fft=n_fft, hop_length=hop_len, win_length=win_len, window=self.window, return_complex=True)
            X = X.permute(0, 2, 1)  # [B, T, F]
            mag_square_X = th.abs(X) ** 2
            Ex = th.matmul(mag_square_X, self.fbank.T)
            Y = th.stft(echo_ref_sig, n_fft=n_fft, hop_length=hop_len, win_length=win_len, window=self.window, return_complex=True)
            Y = Y.permute(0, 2, 1)  # [B, T, F]
            mag_square_Y = th.abs(Y) ** 2
            Ey = th.matmul(mag_square_Y, self.fbank.T)
            echo_aware = Ey / (Ex + Ey + 1e-8)
            
            mycost_loss = mycost(gains_label, outputs["gains"], echo_aware)
            ...
            reduce_asym_loss = reduce_mean(asym_loss, self.world_size)
            ...
            loss_dict['ASYMLOSS'] = reduce_asym_loss.item()
