...
def test_model():
    model = EchoFree(in_dim=112, out_dim=100)
    ...
    # print(f"#macs: {flops / 10.0**6}")

    from ptflops import get_model_complexity_info
    macs, params = get_model_complexity_info(model, (2,16000), as_strings=False, print_per_layer_stat=False, verbose=True)
    print(macs / 10.0**6)

    # from fvcore.nn import FlopCountAnalysis, parameter_count
    # flop_analyzer = FlopCountAnalysis(model, input_feature)
    # print(f"FLOPs: {flop_analyzer.total()}")  # 计算量
